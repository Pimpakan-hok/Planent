...
export default function PostEditor({ route, navigation }) {
  ...
  async function currentDataLoader() {
    let firecall = firebase.firestore().collection('events').doc(evpoid.eid).collection('posts').doc(evpoid.poid);

    await firecall.get().then((querySnap) => {
      setCurdata(querySnap.data());
      setDefaultdata(querySnap.data());
    });
  }

  function setDefaultdata(post) {
    setPic(post.picture);
  ...
  useEffect(() => {
    ...
    let firecall = firebase.firestore().collection('users').doc(uid);

    firecall.get().then((querySnap) => {
      setUdata(querySnap.data());
    })
    
    askPer();
    //loadImage();
    return () => {setCurdata([]); setDefaultdata([]);};
  }, []);
  ...
  async function insertToDatabase() {
    try {
      let collRef = firebase.firestore().collection("events").doc(evpoid.eid).collection('posts').doc(evpoid.poid);
